    ...
    def prepare_module(self):
        ...
        if self.has_post_config_hook:
            try:
                ...
                self.error_output(self.error_messages[0])
                msg = (
                    "Exception in `%s` post_config_hook()"
                    % self.module_full_name
                )
                self._py3_wrapper.report_exception(msg, notify_user=False)
                self._py3_wrapper.log(
                    "terminating module %s" % self.module_full_name
                )
        self.enabled = True
    ...
    def force_update(self):
        ...
        for meth in self.methods:
            ...
            if self.config["debug"]:
                self._py3_wrapper.log(
                    "clearing cache for method {}".format(meth)
                )
        ...
        try:
            if self.error_messages:
                ...
                if button == 1:
                    ...
                    self.error_index = (self.error_index + 1) % len(
                        self.error_messages
                    )
        ...
        if self._py3_wrapper.running:
            ...
            for meth, obj in self.methods.items():
                ...
                try:
                    ...
                    if isinstance(
                        response.get("full_text"), (list, Composite)
                    ):
