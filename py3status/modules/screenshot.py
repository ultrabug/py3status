# -*- coding: utf-8 -*-
"""
Display a 'SHOT' button in your i3bar allowing you to take screenshot and directly send (if wanted) the file to your online server, as long the screenshot 
  has been taken, 'SHOT' is replaced by the file_name generated 
...
"""
import time, os, string, random, subprocess
...
class Py3status:
  ...
  def on_click(self, i3s_output_list, i3s_config, event):
    ...
    if self.push and self.upload_server and self.upload_user and self.upload_path:
      command = "scp %s/%s%s %s@%s:%s" % (self.save_path, file_name, '.jpg', self.upload_user, self.upload_server, self.upload_path)
      subprocess.Popen(command.split())
