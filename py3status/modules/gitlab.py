...
Format placeholders:
    See `sp` and `pipe` below for an explicit list of supported GitLab placeholders to
    ...
    pipelines:
        {pipelines_status}          global status of pipelines, eg success
...
class Py3status:
    ...
    button_refresh = 2
    cache_timeout = 900
    format = "{name} [[{open_issues_count}][\?soft /]" "[{open_merge_requests_count}]]"
