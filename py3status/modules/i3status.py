...
Configuration parameters:
    ...
    modules: specify a list of i3status modules and settings, optionally
        with `format_module` using `{output}` placeholder (default [])
...
STRING_NO_MODULES = "missing `modules`"
STRING_ERR_MODULES = "invalid `modules`"
...
class Py3status:
    ...
    def post_config_hook(self):
        ...
        if not self.modules:
            raise Exception(STRING_NO_MODULES)
        for module in self.modules:
            if not isinstance(module, dict) or not module.get("name"):
                raise Exception(STRING_ERR_MODULES)
    ...
    def _write_i3status_config(self):
        ...
        for module in self.modules:
            ...
            settings = {k: v for k, v in module.items() if k not in ("name", "format_module")}
            if not settings:
                continue
    ...
    def _format_modules(self):
        ...
        for item, module in zip(self.items, self.modules):
            ...
            format_module = self.py3.safe_format(module["format_module"], {"output": item})
            new_modules.append(format_module)
        return new_modules
    ...
    def _set_error(self, error):
        ...
        self.error = error
        self.py3.log(self.error, level="error")
            ...
            while self.running:
                ...
                if not line:
                    if self.process.poll() is not None:
                        ...
                        break
                    continue
                items = self._parse_i3status_line(line.strip())
