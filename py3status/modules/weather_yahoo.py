...
Configuration parameters:
    ...
    - format : uses 2 placeholders:
        ...
        - {forecasts} : text generated by `forecast_format`, separated by
...
class Py3status:
    ...
    def weather_yahoo(self, i3s_output_list, i3s_config):
        ...
        today_text = self.today_format.format(icon=self._get_icon(today),
                                              units=units, **today)
        forecast_text = self.forecast_text_separator.join(
            self.forecast_format.format(icon=self._get_icon(f), units=units,
                                        **f)
            for f in forecasts)
