...
class ConfigParser:
    ...
    * quality feedback on parse errors.
    ...
    def tokenize(self, config):
        ...
        for token in re.finditer(reg_ex, config):
            ...
            if token.group("operator"):
                ...
                continue
            tokens.append(
                {"type": t_type, "value": value, "start": token.start()}
            )
        self.tokens = tokens
    ...
    def make_value_from_shell(self, value):
        ...
        match = re.match(
            "shell\((.+?)(\s*,\s*(int|float|str|bool|auto))?\)", value
        )
        shell, _, var_type = match.groups()
        try:
            shell_stdout = str(
                check_output(shell, shell=True).rstrip(), encoding="utf-8"
            )
