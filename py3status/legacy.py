...
# fmt: off
_I3STATUS_INSTANCE_MODULE_NAMES = {
    "battery", "cpu_temperature", "disk", "ethernet", "memory", "path_exists",
    "read_file", "run_watch", "tztime", "volume", "wireless",
}
_I3STATUS_SINGLE_NAME_MODULES = {"cpu_usage", "ddate", "ipv6", "load", "time"}
_I3STATUS_TIME_MODULE_NAMES = {"time", "tztime"}
_I3STATUS_TIME_FORMAT = "%Y-%m-%d %H:%M:%S"
_I3STATUS_TZTIME_FORMAT = "%Y-%m-%d %H:%M:%S %Z"
_I3STATUS_COLOR_OPTION_NAMES = {"color_bad", "color_good", "color_degraded"}
_I3STATUS_MODULE_COLOR_NAMES = {"general", "battery", "cpu_temperature", "disk", "load"}
_I3STATUS_WRAPPER_SETTING_NAMES = {
...
def is_i3status_wrapper_section(name):
    ...
    return bool(split_name) and split_name[0] in (
        _I3STATUS_SINGLE_NAME_MODULES | _I3STATUS_INSTANCE_MODULE_NAMES
    )


def get_i3status_wrapper_section_name_error(name):
    split_name = name.split()
    if len(split_name) > 1 and split_name[0] in _I3STATUS_SINGLE_NAME_MODULES:
        return "Invalid name cannot have 2 tokens"
    return None


def _get_i3status_generated_name(name):
    return f"i3status {'-'.join(name.split())}_generated"


def _rewrite_i3status_section_references(section_config, get_generated_name):
...
def _is_i3status_wrapper_config_key(module_name, key):
    if key.startswith(".") or key in _I3STATUS_WRAPPER_SETTING_NAMES:
        return True
    if key not in _I3STATUS_COLOR_OPTION_NAMES:
        return False
    return module_name not in _I3STATUS_MODULE_COLOR_NAMES


def _get_i3status_time_interval(module_name, section_config):
    time_format = section_config.get("format")
    if time_format is None:
        if module_name == "time":
            time_format = _I3STATUS_TIME_FORMAT
        else:
            time_format = _I3STATUS_TZTIME_FORMAT
    if "format_time" in section_config:
        time_format = time_format.replace("%time", section_config["format_time"])

    index = 0
    while index < len(time_format):
        index = time_format.find("%", index)
        if index == -1:
            break
        index += 1
        if index == len(time_format):
            break
        if time_format[index] == "%":
            index += 1
            continue
        if time_format[index] in "STXcfrs+":
            return 1
    return 60


def _set_i3status_wrapper_interval(wrapper_config, module_name, section_config):
    if module_name not in _I3STATUS_TIME_MODULE_NAMES:
        return
    general_config = wrapper_config.setdefault("general", {})
    if "interval" not in general_config:
        general_config["interval"] = _get_i3status_time_interval(module_name, section_config)


def _make_i3status_wrapper_config(name, section_config, user_general_config):
    ...
    for key, value in section_config.items():
        if _is_i3status_wrapper_config_key(module_name, key):
            wrapper_config[key] = value
        else:
            module_config[key] = value

    _set_i3status_wrapper_interval(wrapper_config, module_name, section_config)
...
def convert_i3status_wrapper_config(config, user_general_config):
    ...
    rename_map = {
        name: _get_i3status_generated_name(name)
    ...
    for name, section_config in config_sections.items():
        ...
        rewritten_section_config = _rewrite_i3status_section_references(
            section_config, get_generated_name
        )
        if generated_name == name:
            resolved_section_config = rewritten_section_config
        else:
            resolved_section_config = _make_i3status_wrapper_config(
