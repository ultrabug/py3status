...
class Common:
    ...
    def report_exception(
        self, msg, notify_user=True, level="error", error_frame=None
    ):
        """
...
class Py3statusWrapper:
    ...
    def notify_user(
        ...
        if level != "info" and module_name == "":
            fix_msg = (
                u"{} Please try to fix this and reload i3wm (Mod+Shift+R)"
            )
        ...
        msg_hash = hash(
            u"{}#{}#{}#{}".format(module_name, limit_key, msg, title)
        )
        ...
        try:
            if dbus:
                ...
                nagbar_font = py3_config.get("py3status", {}).get(
                    "nagbar_font"
                )
                if nagbar_font:
                    cmd = [
